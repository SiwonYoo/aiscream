...
export default function ThemeModeToggle() {
  ...
  return (
    <div className="flex items-center gap-3">
      <button type="button" aria-pressed={!isOn} aria-label="다크/라이트 테마 변경" onClick={() => setTheme(isOn ? 'dark' : 'light')} className={`relative h-5 w-9 cursor-pointer rounded-[10px] transition-all duration-300 ${bgClass}`}>
