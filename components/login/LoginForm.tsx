  ...
  return (
    <section className="mt-10 space-y-5 rounded-xl border border-base-stroke bg-white p-5 md:p-7">
      ...
      >
        ...
        <p className={`text-xs text-red-500 transition-opacity md:text-sm ${errorMessage ? 'opacity-100' : 'opacity-0'}`} aria-live="polite">
          {errorMessage || '\u00A0'}
        </p>

        <div className="grid grid-cols-2 items-center gap-2 md:gap-0">
          <button type="submit" disabled={isLoading} className="h-7.5 cursor-pointer rounded-sm bg-black p-1 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 md:h-10 md:text-base">
            로그인
          </button>
