  ...
  return (
    ...
    <header className="flex items-center justify-between border-b border-base-stroke bg-muted p-4 pc:hidden">
      ...
      <button type="button" aria-label="사이드바 토글" className="cursor-pointer">
        <Image src="/assets/images/header/menu.svg" width={18} height={18} alt="" />
      </button>

      {/* 로고 */}
      <h1 className="flex items-center gap-1.5 text-sm">
        <Image src="/assets/images/header/logo.png" width={22} height={22} alt="" aria-hidden />
        <span>AiScReam</span>
      </h1>

      {/* 새 블로그 작성 버튼 */}
      <button aria-label="글 생성하기" className="flex h-6 w-6 cursor-pointer items-center justify-center rounded-sm bg-black">
        <Image src="/assets/images/header/plus.svg" width={10} height={10} alt="" />
      </button>
    </header>
  );
}
